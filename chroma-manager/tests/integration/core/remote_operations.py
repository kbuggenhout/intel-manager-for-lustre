...
class RealRemoteOperations(RemoteOperations):
    ...
    def yum_update(self, server):
        result = self._ssh_address(server['address'], "set -x; date; stat /etc/yum.repos.d/Intel-Lustre-Agent.repo; ls -l /etc/yum.repos.d/Intel-Lustre-Agent.repo; cat /etc/yum.repos.d/Intel-Lustre-Agent.repo; dnf repolist; dnf check-update || true; dnf -y update; rpm -qa | grep -e chroma -e iml || true")
